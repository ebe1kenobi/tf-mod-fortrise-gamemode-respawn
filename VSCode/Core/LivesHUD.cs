...
namespace TFModFortRiseGameModeRespawn
{
  ...
  /// plus rien a intercepter. Ni le prefix ni le postfix ne partaient. Le meme piege a
  /// coute le decompte de PlayTag et le compteur de fleches du variant Corpse.
  ///
  /// Une entite ne fait pas ce pari : Monocle appelle son <c>Render</c> parce qu'elle
  /// est dans la scene, pas parce qu'on a reussi a s'accrocher a une methode. C'est le
  /// meme montage que la jauge de puissance du mod Soccer.
  ///
  /// Elle est dessinee DANS le niveau et non dans un coin de l'ecran : le nombre de
  /// vies restantes se lit au-dessus de l'archer, la ou le regard est deja.
  ///
  /// **Portee.** Elle n'est ajoutee qu'au debut d'une manche de ce mode, et se retire
  /// d'elle-meme si le mode change. Rien n'est dessine dans les autres modes ni dans le
  /// jeu de base.
  /// </summary>
  public class LivesHUD : Entity
  {
    /// <summary>Devant les archers, comme la jauge du foot.</summary>
    public const int DEPTH = -120;

    private const float SEGMENT = 3f;
    private const float GAP = 1f;

    /// <summary>Au-dela, une barre serait plus large que l'archer : on ecrit le nombre.</summary>
    ...
    public override void Render()
    {
      ...
      if (level?.Session == null || !MyRespawnPlayer.IsRespawnMode(level.Session.MatchSettings))
      {
        return;
      }
    ...
    private static void Draw(Player player)
    {
      ...
      // Une seule vie : la barre n'apprendrait rien, il n'y a rien a perdre deux fois.
      if (maxLives <= 1)
      {
        return;
      }
